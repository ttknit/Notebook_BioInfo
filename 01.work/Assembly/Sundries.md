...
bwa=/path/bwa
$bwa index genome.fa
$bwa mem -5SP -t 28 genome.fa Hic.1.clean.fastq.gz Hic.2.clean.fastq.gz | samblaster | samtools view - -@ 48 -S -h -b -F 3340 -o HiC.bam
$bwa mem -5SP -t 28 genome.fa Hic.1.clean.fastq.gz Hic.2.clean.fastq.gz  > hic_bwa.sam
...
haphic pipeline genome.fa HiC.filtered.bam 38 --threads 4 --processes 8 --correct_nrounds 2 # default:RE="GATC"
```

break seq by gci
```bash
bedtools maskfasta -fi /path/genome.fa -bed <(cat /path/GCI_hifi.0.depth.bed /path/GCI_nano.0.depth.bed |sort -k1,1V -k2,2n) -fo /path/masked_lowDP_simplex.fa
...
#SBATCH --mem=30g
ragtag.py patch -o chr${i} -t 48 --aligner minimap2 chr${i}_with_scaffold.fa /path/genome.fasta
ln -s /path//chr${i}/ragtag.patch.fasta /path/chr${i}_ragtag.fa" > job_chr${i}.sh
        sbatch job_chr${i}.sh
done
```
...
```bash
# split fasta
draft=/path/genome.fa
for i in {1..35} X Y;do
        echo "chr${i}" > chr${i}_list.txt
        cat scaffoldNAME.txt >> chr${i}_list.txt
done
for i in {1..35} X Y;do
...
linkview2
```bash
minimap2 -t 16  -x asm10  asm1.fasta asm2.fa >map.paf
python3 alignmentStatFromPaf.py map.paf > map.paf.stat
# rDNA telomere gaps
cat /path/rDNA.filt.withColor.bed /path/v1_rDNA.filt.withColor.bed simplex.full.telomere.200_0.5_m300.bed v1.full.telomere.200_0.5_m300.bed draft_v1.0_gaps_withColor.bed > highlight.bed
sh AssignPlot1asmLinkview.sh asm1.fasta highlight.bed asm2.fa map.paf
```

craq
```bash
craq -g genome.fa -sms v1.0_hifi_pri.bam -t 48 -o craq_out


src=/path/
ref_fa_size=/path/seq.size
p=/path/craq_out
OUTPUT=/path/out_circos.pdf
echo -e "[M::worker_pipeline:: Plot CRAQ metrics]"
cd /path/craq_out
$src/$runAQI_SMS.sh -g  genome.fasta  -z seq.size   -e LRout/LR_eff.size  -C LRout/LR_putative.SE.SH -D LRout/LR_sort.depth  -r 0.75 -p 0.4 -q 0.6 -R 0.75 -P 0.4 -Q 0.6  -n 10 -s 249765 -w 1000000  -j 1 -b T -y T -x /path/craq_out/seq.size -v F
```
...
# stat
awk 'BEGIN{FS=" ";OFS="\t"} $5>0.8{print $1,$3,$5,$6} ' genome.self_dropSelf.paf.stat > self.stat
```

`snakemake -s workflow/Snakefile_test --configfile=${p}/config/config.yaml --config sample=test fasta=/path/genome.fasta samtools_mem=3G --cores 48 --use-conda --rerun-incomplete cooler
`
yahs juicer
```bash
# yahs
...
# juicer
p=/path/yahs/hic_maps
# generate alignment
juicer pre ${p}/../genome.bin ${p}/../genome_scaffolds_final.agp ${p}/../genome.fasta.fai |sort -k2,2d -k6,6d -T ./ --parallel=4 -S24G | awk 'NF' > alignments_sorted.txt.part && mv alignments_sorted.txt.part alignments_sorted.txt
# generate hic
ln -s ${p}/../genome_scaffolds_final.fa .
samtools faidx genome_scaffolds_final.fa
cut -f 1,2 genome_scaffolds_final.fa.fai > scaffolds_final.chrom.sizes
...
python3 /path/alignmentStatFromPaf.py map.paf > map.paf.stat
...
awk 'BEGIN{OFS="";print ",length"}{print ",",$2}' genome.fa.fai > a
...
minimap2  -t 48 -x asm10 genome.p_ctg.fasta chr8a_37l_utg.fa > map_chr8a.paf
bash get_most.sh map_chr8a.paf
awk 'BEGIN{print "Name,v1_name";OFS=","}{print $1,$2}' map_chr8a.paf.stat.most.txt > chr8a_v1name.csv

minimap2  -t 48 -x asm10 genome.p_ctg.fa genome.p_utg.fa > map_ctg.paf
...
join -1 5 -2 5 <(awk '$1~/A/' ../../genome.p_utg.noseq.gfa| sort -k5,5 ) <(awk '$1~/A/' ../../genome.p_ctg.noseq.gfa|sort -k5,5)|cut -f3,11 -d' '|sort -k2|uniq |awk 'BEGIN{OFS=",";print "Name,ptg"}{print $1,$2}' > utgptg_link.csv
```