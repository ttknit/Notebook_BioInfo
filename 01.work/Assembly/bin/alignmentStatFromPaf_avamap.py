...
def main():
    ...
    for q in synteny:
        ...
        for r in synteny[q]:
            ...
            ref_len = scaf_length.get(r, 'N/A')
            ref_alignrate = "{:.5f}".format(nonoverlap_qry_aligned / ref_len)
            two_alignrate = max(alignrate,ref_alignrate)
            info = f"{r}\t{ref_len}\t{nonoverlap_qry_aligned}\t{alignrate}\t{average_identity}\t{ori}\t{two_alignrate}"
...
if __name__ == '__main__':
    ...
    new change: 2025.08.06 Lingxin Qiu
    # qry_id    qry_len ref_id  ref_len map_len alignrate   average_identity    ori two_alignrate_max
    use for reads_ava_map
